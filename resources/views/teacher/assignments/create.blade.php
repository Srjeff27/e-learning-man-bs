...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 class="text-xl font-semibold text-slate-900 mb-6">Buat Tugas Baru</h2>

            <form action="{{ route('teacher.assignments.store', $classroom) }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="instructions" name="instructions" rows="4" class="input"
                        placeholder="Petunjuk detail bagaimana mengerjakan tugas ini">{{ old('instructions') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="submission_type" name="submission_type" class="input" required>
                        <option value="file" {{ old('submission_type') === 'file' ? 'selected' : '' }}>Upload File</option>
                        <option value="text" {{ old('submission_type') === 'text' ? 'selected' : '' }}>Teks/Essay</option>
                        <option value="link" {{ old('submission_type') === 'link' ? 'selected' : '' }}>Link URL</option>
                        <option value="multiple" {{ old('submission_type') === 'multiple' ? 'selected' : '' }}>Kombinasi</option>
                    </select>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div id="late_penalty_wrapper" class="pl-6 {{ old('allow_late_submission') ? '' : 'hidden' }}">
                        ...
                        <input type="number" id="late_penalty_percent" name="late_penalty_percent" 
                            value="{{ old('late_penalty_percent', 10) }}" class="input w-32" min="0" max="100">
                    </div>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    <label for="is_published" class="ml-2 text-sm text-gray-700">Publikasikan tugas sekarang</label>
                </div>
