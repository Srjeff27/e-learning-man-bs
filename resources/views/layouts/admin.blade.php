        ...
        <aside :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}"
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto scrollbar-thin">
                ...
                <a href="{{ route('admin.teachers.index') }}"
                    class="nav-item flex items-center px-4 py-3.5 rounded-xl relative {{ request()->routeIs('admin.teachers.*') ? 'active' : '' }}">
                    <span class="nav-indicator"></span>
                    <div
                        class="nav-icon w-10 h-10 rounded-xl flex items-center justify-center mr-3 bg-slate-100 dark:bg-slate-800/50">
                        <svg class="w-5 h-5 {{ request()->routeIs('admin.teachers.*') ? 'text-white' : 'text-emerald-500 dark:text-emerald-400' }}"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                    </div>
                    <span class="font-semibold text-slate-700 dark:text-slate-200 text-sm">Guru & Staff</span>
                </a>
