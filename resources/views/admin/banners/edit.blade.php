...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Pengumuman</h1>
            ...
            <form action="{{ route('admin.banners.update', $banner) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" required
                        ...
                        placeholder="Contoh: PPDB Tahun Ajaran 2025/2026 Telah Dibuka"
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="4" required
                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                        placeholder="Tuliskan isi pengumuman secara singkat dan jelas...">{{ old('content', $banner->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="link_text" id="link_text"
                            ...
                            value="{{ old('link_text', $banner->link_text ?? 'Selengkapnya') }}">
                        @error('link_text')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
