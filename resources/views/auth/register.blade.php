...
@section('content')
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div class="space-y-1.5">
            <label for="name" class="block text-sm font-semibold text-slate-700 dark:text-slate-200">Nama Lengkap</label>
            <div class="relative group">
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}"
                    class="input-glass @error('name') !border-red-500 @enderror" placeholder="Masukkan nama lengkap"
        ...
        <div class="space-y-1.5">
            ...
            <div class="relative group" x-data="{ show: false }">
                ...
                <input id="password" :type="show ? 'text' : 'password'" name="password"
                    class="input-glass pr-12 @error('password') !border-red-500 @enderror" placeholder="Min. 8 karakter"
        ...
        <div class="space-y-1.5">
            <label for="password_confirmation"
                class="block text-sm font-semibold text-slate-700 dark:text-slate-200">Konfirmasi Password</label>
            <div class="relative group" x-data="{ show: false }">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <svg class="w-5 h-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors duration-300"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <input id="password_confirmation" :type="show ? 'text' : 'password'" name="password_confirmation"
                    class="input-glass pr-12" placeholder="Ulangi password" required autocomplete="new-password">
                <button type="button" @click="show = !show"
                    class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-emerald-500 transition-colors duration-300">
                    <svg x-show="!show" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex items-start">
            <input type="checkbox" name="terms" id="terms" required
                class="mt-0.5 w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-emerald-600 focus:ring-emerald-500 focus:ring-offset-0 transition-colors">
            <label for="terms" class="ml-2.5 text-sm text-slate-600 dark:text-slate-400 cursor-pointer">
                Saya menyetujui
                <a href="#" class="font-medium text-emerald-600 dark:text-emerald-400 hover:underline">Syarat &
                    Ketentuan</a>
                dan
                <a href="#" class="font-medium text-emerald-600 dark:text-emerald-400 hover:underline">Kebijakan Privasi</a>
            </label>
        </div>
